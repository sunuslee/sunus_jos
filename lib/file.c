...
open(const char *path, int mode)
{
        ...
        if((r = fsipc(FSREQ_OPEN, (void *)pfd)) < 0)
        {
                fd_close(pfd, 0);
                return r;
        }
//*        pdata = (struct Page *)fd2data(pfd);
//*        if((r = sys_page_alloc(0 , pdata, PTE_USER)) < 0);
//*        {
//*                fd_close(pfd, 0);
//*                return r;
//*        }
...
devfile_read(struct Fd *fd, void *buf, size_t n)
{
        ...
        while(bytes_left > 0)
        {
                ...
                fsipcbuf.read.req_n = bytes_left > PGSIZE ? PGSIZE : bytes_left;
...
devfile_write(struct Fd *fd, const void *buf, size_t n)
{
        ...
        int bytes_left = n;
        int r;
        int max = sizeof(fsipcbuf.write.req_buf) - 1;
        while(bytes_left > 0)
        {
                fsipcbuf.write.req_fileid = fd->fd_file.id;
                fsipcbuf.write.req_n = bytes_left > max ? max : bytes_left;
