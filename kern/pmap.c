...
i386_vm_init(void)
{
        ...
        // sunus,Dec 5,2010
        envs = boot_alloc(NENV * sizeof(struct Env), 4096);
        ...
        // map envs to UENVS , PREM is user read only 
        boot_map_segment(pgdir, UENVS, NENV * sizeof(struct Env),(physaddr_t)PADDR(envs),PTE_U);
...
page_init(void)
{
        ...
        LIST_INIT(&page_free_list);
        /*
           cprintf("step_nr is %u\n",basemem/PGSIZE);		
           for(i = 1,step_nr = basemem/PGSIZE ; i < step_nr ; i++)
           {
           pages[i].pp_ref = 0;
           LIST_INSERT_HEAD(&page_free_list, &pages[i], pp_link);
           } 
           cprintf("step_nr is %u\n",(EXTPHYSMEM - IOPHYSMEM)/PGSIZE);		
           for(step_nr = (EXTPHYSMEM - IOPHYSMEM) / PGSIZE ; step_nr; step_nr--, i++)
           {	
           pages[i].pp_ref = 1;
           }
         */
        cprintf("basemem = %08x,IOPHYSMEM = %08x,EXTPHYSMEM = %08x,extmem = %08x\n",basemem,IOPHYSMEM,EXTPHYSMEM,extmem);
        cprintf("KER=%08x,PA-KER=%08x\nboot_freemem = %08x,PA-boot_freemem = %08x\n",KERNBASE,PADDR(KERNBASE),boot_freemem,PADDR(boot_freemem));
        for(i = 1 ; i < npage ; i++)
        {
                pages[i].pp_ref = 0;
                if((i >= IOPHYSMEM/PGSIZE && i < EXTPHYSMEM/PGSIZE) 
                                || (i >= PADDR(KERNBASE)/PGSIZE && i < (size_t)ROUNDUP(PADDR(boot_freemem),PGSIZE)/PGSIZE))
                {
                        pages[i].pp_ref = 1;
                        continue;
                }
...
page_free(struct Page *pp)
{
        ...
        if(pp->pp_ref == 0)
        {
                ...
                // i dont know why i cant use this : page_initpp(pp);
                // ANSWER : page_initpp is to memset THIS STRUCT to 0 , No Page Memory to 0 !
        }
        /* SUNUS,Nov,20, 2010 */
}
...
{ // Fill this function in //SUNUS,20,Nov 2010
        pte_t *pte;
        struct Page *p;
        pgdir = &pgdir[PDX(va)]; /* 10-bits address */	    
        if(pgdir && (*pgdir & PTE_P) == 0)
        {
                if(create == 0)
                        return NULL;
                else
...
page_remove(pde_t *pgdir, void *va)
{
        ...
        pde_t *p = NULL;
        struct Page *pp;
        p =(pde_t *)&pgdir[PDX(va)];
        pp = page_lookup(pgdir, va, &p);
        if(pp != NULL)
        {
                page_decref(pp);
                *p = 0;
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
        // LAB 3: Your code here. 
        // DEC 15,2010 sunus
        uintptr_t va_end = ROUNDUP((uintptr_t)va + len - 1,PGSIZE);
        ...
        for( ; va_start < va_end ; va_start += PGSIZE)
        {
                ...
                pt = pgdir_walk(env->env_pgdir, (void *)va_start, 0);
                if(!pt || !(*pt & (perm|PTE_P|PTE_U|PTE_W)))
                {
                        sunus_dbg(SUNUSDBG,1,perm,"I AM 835");
                        cprintf("pt = %08x, *pt = %08x\n",(uint32_t)pt,(uint32_t) (pt == 0 ? 0 : *pt));
                        user_mem_check_addr = va_start;
                        return -E_FAULT;
                }
        }
